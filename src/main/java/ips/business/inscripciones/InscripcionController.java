	...
	public int[] comprobarPagosDeCarrera(int idCarrera) {
		...
		try {
			...
			while(linea != null) {
				...
				// Ahora mismo tengo la cantidad del pago que ha hecho y la fecha en la que se ha realizado
				...
				if(Double.valueOf(campos[2]) > precioCarrera) {
					...
					//System.out.println("pagaste mas" + inscripcion.get(0).getDnicorredor() + idCarrera + "Devolver " + diferencia + " euros");
					modelBanco.anotarIncidencia(inscripcion.get(0).getDnicorredor(), idCarrera, "Pagó más, devolver " + diferencia + " euros");
				
				// si es menos la inscripcion sera anulada (update a anulada) y anotar en incidencias que hay que
				// devolver x euros
				} else if(Double.valueOf(campos[2]) < precioCarrera) {
					//System.out.println("pagaste menos" + inscripcion.get(0).getDnicorredor() + idCarrera + "Devolver " + campos[2] + " euros");
					modelBanco.anotarIncidencia(inscripcion.get(0).getDnicorredor(), idCarrera, "Pagó menos, devolver " + campos[2] + " euros");
				...
				if(LocalDate.parse(campos[1]).compareTo(LocalDate.parse(inscripcion.get(0).getFechainscripcion()).plusDays(2)) > 0) { // Ha pagado más de 48h después
					//System.out.println("El pago se ha hecho más de 48h después");
					//System.out.println("" + inscripcion.get(0).getDnicorredor() + idCarrera + "Devolver " + campos[2] + " euros");
					modelBanco.anotarIncidencia(inscripcion.get(0).getDnicorredor(), idCarrera, "Pagó fuera de plazo, devolver " + campos[2] + " euros");
					modelBanco.updateInscriptionAnular(inscripcion.get(0).getDnicorredor(), idCarrera);
					resultado[1] = resultado[1]+1;
					linea = reader.readLine();
					continue;
		...
		// despues de comprobar los pagos, pillar todos los que esten en pendiente de pago, verificar si han pasado mas de 48h desde que se han inscrito,
		...
		for(InscripcionDTO pendiente: pendientesDePago) {
			if(LocalDate.now().compareTo(LocalDate.parse(pendiente.getFechainscripcion()).plusDays(2)) > 0) { // No ha pagado en 48h
				System.out.println("El atleta " + pendiente.getDnicorredor() + " inscrito en la carrera " + idCarrera + " no ha pagado en 48h, se le anula inscripcion");
