...
public class MenuCrearCarreraController {
	...
	private boolean checkCamposCrearCarrera() {
		...
		if (view.getTxNombre().getText().isBlank() || view.getTxLugar().getText().isBlank()
				...
				|| view.getFechaCompeticion().getDate() == null || !view.getFechaCompeticion().getDate().after(fechaActual)
				|| isPlazosNoListo())
			return false;
		return true;
	}
	...
	private ActionListener añadirPlazo() {
		return new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				VentanaCrearPlazoController plazoController = new VentanaCrearPlazoController(plazoView,new java.sql.Date(view.getFechaCompeticion().getDate().getTime()));
